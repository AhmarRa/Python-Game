...
def create_enemy(world):
    if world == "North World":
        enemies = [
            {"name": "Frost Giant", "health": 80},
            {"name": "Ice Dragon", "health": 80},
            {"name": "Snow Wolf", "health": 75},
            {"name": "Frozen Ogre", "health": 70},
            {"name": "Blizzard Bear", "health": 75}
        ]
    elif world == "South World":
        enemies = [
            {"name": "Forest Spirit", "health": 70},
            {"name": "Tree Ent", "health": 70},
            {"name": "Poisonous Vine", "health": 75},
            {"name": "Enchanted Boar", "health": 75},
            {"name": "Cursed Dryad", "health": 70}
        ]
    elif world == "East World":
        enemies = [
            {"name": "Fire Phantom", "health": 80},
            {"name": "Lava Beast", "health": 85},
            {"name": "Cinder Wraith", "health": 70},
            {"name": "Flame Scorpion", "health": 80},
            {"name": "Molten Serpent", "health": 85}
        ]
    elif world == "West World":
        enemies = [
            {"name": "Stone Golem", "health": 80},
            {"name": "Earth Elemental", "health": 75},
            {"name": "Rock Serpent", "health": 85},
...
def generate_loot(enemy, character):
    ...
    # Add 10 health points if a relic is won
    if relic_won:
        character["health"] += 10
        if character["health"] > character["max_health"]:
            character["health"] = character["max_health"]
        loot.append(f"{character['name']} gains 10 health points! Current health: {character['health']}")
...
def handle_victory(character, enemy):
    ...
    # Restore 15 health points after defeating the enemy
    character["health"] += 15
    if character["health"] > character["max_health"]:
        character["health"] = character["max_health"]

    print(f"{character['name']} restores 15 health points! Current health: {character['health']}")
